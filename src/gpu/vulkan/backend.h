...
namespace ember::gpu
{
	...
	struct DeviceBackend
	{
		...
		vk::Queue graphics{}; // owns submission; also presents (checked at adapter selection)
		vk::Queue compute{};  // dedicated async-compute family when the adapter has one, else == graphics
		vk::Queue transfer{}; // dedicated DMA family when present, else == graphics
		...
		vk::FrameSlot slots[MAX_FRAMES_IN_FLIGHT]{};
		...
		std::array<vk::PendingPresent, MAX_SWAPCHAINS> pending_presents{};
